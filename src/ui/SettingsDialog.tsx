      ...
      <div key={connection.id} className={`rounded-xl border bg-card/70 transition-colors ${isActive ? 'border-primary/50 shadow-soft' : 'border-border/70'}`}>
        ...
        {connection.expanded && (
          <div className="space-y-4 border-t px-4 py-4">
            ...
            <div>
              <div className="mb-2 flex items-center justify-between"><Label>模型列表</Label><div className="flex gap-2"><Button size="sm" variant="ghost" onClick={() => settings.addModel(connection.id)}><Plus className="h-3.5 w-3.5" /> 新建</Button><Button size="sm" variant="ghost" onClick={() => settings.removeModel(connection.id, connection.models[connection.models.length - 1]?.uiKey ?? `${connection.id}-model-${connection.models.length - 1}`)} disabled={connection.models.length <= 1}>删除所有</Button></div></div>
              <div className="space-y-2">
                {connection.models.map((model, modelIndex) => <div key={model.uiKey ?? `${connection.id}-model-${modelIndex}`} className="grid grid-cols-[1fr_1fr_auto_auto] gap-2"><Input value={model.name} onChange={(e) => settings.updateModel(connection.id, model.uiKey ?? `${connection.id}-model-${modelIndex}`, { name: e.target.value })} placeholder="显示名称" /><Input value={model.id} onChange={(e) => settings.updateModel(connection.id, model.uiKey ?? `${connection.id}-model-${modelIndex}`, { id: e.target.value })} placeholder="Model ID" /><Button size="sm" variant={isActive && settings.activeModelId === model.id ? 'secondary' : 'outline'} onClick={() => { settings.setActiveConnection(connection.id); settings.setActiveModel(model.id); }}>{isActive && settings.activeModelId === model.id ? '当前' : '使用'}</Button><Button size="icon" variant="ghost" aria-label="删除模型" disabled={connection.models.length <= 1} onClick={() => settings.removeModel(connection.id, model.uiKey ?? `${connection.id}-model-${modelIndex}`)}><Trash2 className="h-4 w-4" /></Button></div>)}
              </div>
            </div>
